...
import { MedicationForm, MedicationFormComponent } from '../../forms/medication-form/medication-form.component';
import { SymptomForm, SymptomFormComponent } from '../../forms/symptom-form/symptom-form.component';
...
export class DayViewComponent implements OnInit {
  ...
  deleteTask(t: Task) {
    this.day.tasks.forEach((task,index)=>{
      if(task==t) this.day.tasks.splice(index,1);
   });
  }
  ...
  deleteSymptoms(s: Symptom) {
    this.day.symptoms.forEach((sym,index)=>{
      if(sym==s) this.day.symptoms.splice(index,1);
   });
  }
  ...
  deleteMedications(m: Medication) {
    this.day.medications.forEach((med,index)=>{
      if(med==m) this.day.medications.splice(index,1);
   });
  }
  toggleEdit(): void {
